...
export default function Home() {
  ...
  const handleCheckboxChange = (file: File) => {
    ...
    if (selectedTempFiles.includes(file)) {
      updatedSelectedTempFiles = selectedTempFiles.filter(
        (item) => item !== file,
      );
    } else {
    ...
    <>
      <div className="flex flex-row">
        ...
        <div
          className={
            nav
              ? 'h-full fixed z-20 w-[85%] bg-slate-200 px-4 py-5  flex flex-col justify-between space-y-5 items-center'
              : 'hidden absolute left-[-100%]'
          }
        >
          <button
            onClick={handleNav}
            className="absolute top-0 right-0 bg-blue-400 p-2 w-8 h-8 flex items-center justify-center"
          >
            <svg
            ...
            <>
              <div>
                <div className="rounded-md mx-4 bg-black text-white px-2 py-2 mb-2 text-center flex justify-between">
                  <h1 className="mr-3">Filter from Documents</h1>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="20"
                    height="20"
                ...
                <ul className="text-black px-4 overflow-y-auto h-[500px] custom-scrollbar">
                  {allfiles.map((file) => (
                    <li
                      className="space-x-6 flex items-center my-1 justify-between border-b-2 border-gray-300 py-1"
                      key={file}
                    >
              ...
              <div className="flex flex-row w-full justify-center mt-5">
                <button
                  onClick={handleNav}
                  className="rounded-md mx-4 bg-blue-400 text-white px-4 py-2 mb-2 hover:bg-blue-500"
                >
                  Done
                </button>
              </div>
        ...
        {filterOptionEnabled ? (
          <>
            <div className="hidden md:flex h-screen w-[400px] bg-slate-200 px-4 py-10  md:flex-col justify-between space-y-5">
              <div>
                <div className="rounded-md mx-4 bg-black text-white px-2 py-2 mb-2 text-center">
                  Filter from Documents
                </div>
                {/* <h1 className=" text-center text-black font-bold my-3">Filter from Uploaded Documents</h1> */}
                <ul className="text-black px-4 overflow-y-auto h-[500px] custom-scrollbar">
                  {allfiles.map((file) => (
        ...
        <Layout>
          <div className="mx-auto flex flex-col gap-4">
            <div className="w-full flex flex-row justify-between md:justify-center p-4 items-center">
              <button
                onClick={handleNav}
                className="bg-gray-900 p-2 flex items-center justify-center md:hidden"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
